...
A centralized collection of ThayerLab publications, theses, and research materials, along with curated links to relevant external literature.

## Current knowledge graph

<!-- KG:START -->
The knowledge graph has not been generated yet. After the first successful ingestion, this section will contain the graph image and a table linking each document to its contents, keywords, and connected documents.
<!-- KG:END -->

## Project status and scope

**This README is an implementation specification, not a description of working automation.** The current Python scripts and `KG/KG.graphml` are empty placeholders; the ingestion workflow still needs to be implemented.
...
The initial scope is a small collection processed through GitHub Actions and browsed on GitHub. A web application, graph database, embeddings, automatic literature downloads, citation extraction, and OCR are outside this version. `external_papers.md` and `thayerlab_papers_theses.md` remain manually curated indexes; links in them do not trigger ingestion.
...
KG/
  ...
  remove_node.py                 Reserved for later explicit removal support
...
`metadata.json`, `content.md`, archived sources, and `relationships.json` are the persistent records. GraphML, the image, and the generated README section must be rebuildable from these records without LLM calls. Do not make `remove_node.py` part of normal ingestion.
...
7. **Render the README.** Replace only the text between the standalone KG generation markers near the top of this README. Put the image first, followed by a document table containing titles, links to content and original sources, keywords, and linked neighbors. Include edge rationales in a connection table. Escape Markdown-special characters and encode link paths. Missing or duplicate markers are errors; never rewrite the hand-maintained plan.
...
- The graph image, document links, and connection rationales are visible near the top of this README, with inferred edges clearly identified.
