...
def convert_pdf(data: bytes, config: dict) -> tuple[str, dict]:
    try:
        ...
        for number, page in enumerate(reader.pages, 1):
            ...
            text = page.extract_text(extraction_mode="layout").strip() if stream is not None else ""
            # Layout mode can miss text inside Form XObjects (e.g. thesis covers).
            if not text:
        ...
        tool = "pypdf-layout-with-plain-fallback" if used_plain_fallback else "pypdf-layout"
        return "\n".join(pages), {"tool": tool, "version": pypdf.__version__}
    except KBError:
        raise
